...
const handleSubmit=(e)=>{
  ...
  return (
    ...
    <div className={`${moveToHome ? "hidden" :"flex" } flex-col  h-screen  bg-black text-green-500`}  >
        <form className={`${!moveToHome ? "flex" : "hidden"}  p-2 text-center text-xl flex flex-col w-100 m-auto gap-3 text-black`} onSubmit={handleSubmit}>
        <img src={myself} className='h-20 z-20 w-20 rounded-full  m-auto'/>

            <header className='text-center text-2xl font-extrabold text-white '>SIGN UP</header>
            <input className=' p-1 rounded-xl border-2 border-grey-100 ' required name='firstName' onChange={handleChange} value={SignUpDetails.firstName}  type='text' placeholder='First Name'/>
            <input className=' p-1 rounded-xl border-2 border-grey-100 ' required name='lastName'  onChange={handleChange} value={SignUpDetails.lastName} type='text' placeholder='Last Name'/>
            <input className=' p-1 rounded-xl border-2 border-grey-100 ' required name='email' onChange={handleChange} value={SignUpDetails.email} type='email' placeholder='Email'/>
            <input className=' p-1 rounded-xl border-2 border-grey-100 ' required name='password'  onChange={handleChange} value={SignUpDetails.password} type='password' placeholder='Password'/>
            <button type='submit' className='bg-pink-500 hover:bg-pink-700 text-white tracking-wider rounded-md p-1 w-1/2 mt-2 m-auto'>Sign Up</button>
